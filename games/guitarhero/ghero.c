...
int KEY2 = ALLEGRO_KEY_2;
int KEY3 = ALLEGRO_KEY_3;
...
void draw_scenario(ALLEGRO_DISPLAY *display)
{
	...
	al_draw_text(SIZE_TITLE, al_map_rgb(255, 0, 0), SCREEN_W / 2 - 150, SKY_H / 4, 0, "G-HERO");
...
void createRandomTile(Tile tiles[], int idx)
{
	...
	if (count_tiles_created < TOTAL_TILES)
	{
		...
		tiles[idx].x = TRACK_LEFT_X + LANE_W * tiles[idx].lane + LANE_W / 2.0;
		tiles[idx].color = COLORS[tiles[idx].lane];
